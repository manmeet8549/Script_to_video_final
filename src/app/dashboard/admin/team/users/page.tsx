...
import { Search, Plus, Mail, ShieldAlert } from "lucide-react";
...
export default function AdminTeamUsersPage() {
  ...
  const filteredMembers = members.filter((m) => {
    const matchesRole = m.role === "viewer" || m.role === "admin";
    const matchesSearch =
    ...
    <main className="flex-1 overflow-y-auto px-8 py-8 bg-zinc-50/50">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-end gap-4 bg-white border border-zinc-200 p-4 rounded-2xl shadow-2xs select-none">
          <span className="text-xs font-bold text-zinc-400">Showing 5 members</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMembers.map((member) => (
            <div
              key={member.id}
              className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm space-y-4 hover:border-zinc-350 transition-all flex flex-col justify-between"
            >
              ...
              <div className="text-left space-y-1.5">
                ...
                <span className={`inline-block px-2.5 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wide mt-1.5 ${
                  member.role === "admin"
