...
export default function UserNotificationsPage() {
  ...
  const markAsRead = async (id: string, currentlyRead: boolean) => {
    ...
    try {
      ...
      // Fail silently for single-item click
    }
  };
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 pb-4">
          <div className="text-left leading-normal">
            ...
            <p className="text-sm font-semibold text-zinc-400 mt-0.5">
              Stay updated with project workflows, review approvals, and system alerts.
            </p>
          </div>
          {notifications.some((n) => !n.is_read) && (
            <button
              onClick={markAllAsRead}
              className="h-9 px-4 bg-blue-50 hover:bg-blue-100 text-blue-650 text-xs font-bold rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer border border-blue-100"
            >
              <Check size={14} />
              Mark all read
            </button>
            ...
            {notifications.map((n) => (
              <div
                ...
                className={`bg-white border rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-all flex items-start justify-between gap-6 text-left cursor-pointer ${
                  n.is_read ? "border-zinc-200 opacity-75" : "border-blue-200 ring-1 ring-blue-500/5"
                }`}
              >
